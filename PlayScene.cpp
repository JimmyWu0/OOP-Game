...
#include "GiantArmy.hpp"  //DONE
#include "FrozenArmy.hpp"  //DONE
#include "HeroArmy.hpp"  ////

// Defense
#include "CannonDefense.hpp"
#include "WallDefense.hpp"
#include "SlowerDefense.hpp"  //DONE
...
void PlayScene::Initialize() {
	...
	timer = al_create_timer(1.0f);  ////initialization of Frozen spell
	lose_timer = al_create_timer(1.0f);  ////
	al_start_timer(lose_timer);
	al_set_timer_count(lose_timer, 0);
...
void PlayScene::Update(float deltaTime) {
    ...
    // Win
	int flag = 1;
	Defense* tgt5 = nullptr;
	for (auto& it : DefenseGroup->GetObjects())
	{
		tgt5 = dynamic_cast<Defense*>(it);
		if (tgt5->id != 4)
		{
			flag = 0;
			break;
	...
	//if (DefenseGroup->GetObjects().empty())  ////modified too
	if(flag)
	{  
		al_stop_timer(lose_timer);
		al_set_timer_count(lose_timer, 0);
        Engine::GameEngine::GetInstance().ChangeScene("win");
    }
    
    // Lose  //TODO, in last part, deal with time's up
    bool armyEmpty = true;
    for (int i=0; i<totalArmy, i != 4; i++) {
        if (armyAmount[i] > 0) {
            armyEmpty = false;
            break;
        }
	...
	if (al_get_timer_count(lose_timer) == 60)  ////lose time
	{
		armyEmpty = true;
		al_stop_timer(lose_timer);
		al_set_timer_count(lose_timer, 0);
	...
	if (button & 1) {
		if (!CheckOccupied(x, y)) {
            ...
            if (remainId != -1) {
				...
				if (remainId == 2)
					preview = new GiantArmy(0, 0);  //DONE
				if (remainId == 3)
					preview = new HeroArmy(0, 0);  ////
				if (remainId == 4)
					preview = new FrozenArmy(0, 0);  ////
...
void PlayScene::OnKeyDown(int keyCode) {
	...
	if (keyCode == ALLEGRO_KEY_TAB) {
		...
		int cnt = 0;
		for (auto i = keyStrokes.begin(); i != keyStrokes.end(); ++i) {
			if (*i != code[cnt])
				break;
			cnt++;
		}
...
void PlayScene::ReadMap() {
	std::string filename = std::string("resources/map") + std::to_string(MapId) + ".txt";  //NOTTODO
	//std::string filename = std::string("resources/map") + ".txt";
	// Read map file.
	char c;
	...
	while (fin >> c) {
		...
		case '3': mapData.push_back(TILE_SLOWER); break;  //TODO
		...
		for (int j = 0; j < MapWidth; j++) {
            ...
            switch (md) {
                ...
                case TILE_CANNON:
                    ...
                    break;
				case TILE_SLOWER:  //DONE
					DefenseGroup->AddNewObject(new SlowerDefense(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));  //DONE
					break;
				case TILE_TRAP:  //DONE
					DefenseGroup->AddNewObject(new TrapDefense(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));  //DONE
					break;
                case TILE_FLOOR:
                    if (j <= MapWidth-2 && j >= 2) {
...
void PlayScene::ConstructUI() {
	...
	ConstructButton(2, ArmyImage[2]);  //TODO
...
bool PlayScene::CheckOccupied(int x, int y) {  //ice
    if (x < 0 || x >= MapWidth  || y < 0 || y >= MapHeight)
        return true;
    ...
    if (tt == TILE_WALL || tt == TILE_CANNON || tt ==TILE_SLOWER) return true;  //TODO
																		//player should be able to place army on the traps since they're invisible
