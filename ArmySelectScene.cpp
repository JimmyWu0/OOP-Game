...
void ArmySelectScene::Initialize() {
    ...
    //armyAmount[0] = 4;
    //armyAmount[1] = 1;
    armyAmount[4] = 2; //Frozen spell

    // set ArmyImage
    ArmyImage[0] = "play/warrior.png";
    // TODO 2 (2/8): Create the bomb image. You can find image in the play/ folder.
    ArmyImage[1] = "play/bomb.png";
    //TODO
    ArmyImage[2] = "play/turret-6.png";
    ArmyImage[3] = "play/turret-1.png";
    ArmyImage[4] = "play/ice.png";
    // Add new enemy
    for (int i=0; i<totalArmy,i!=3; i++) {
        AddNewArmy(i, ArmyImage[i], 1);
    }
    AddNewArmy(3, ArmyImage[3], 3);  //The spaceCost for HeroArmy is 3
    ...
    btn->SetOnClickCallback(std::bind(&ArmySelectScene::PlayOnClick, this, BUTTON_RESET, 0, 0));
    AddNewControlObject(btn);
...
void ArmySelectScene::PlayOnClick(ButtonType type, int id, int spaceCost) {
    if (type == BUTTON_ENTER) {
        ...
        if (usedSpace + spaceCost <= totalSpace)
        {
            ...
            //UISpaceUsage->Text = "Space: " + std::to_string(usedSpace) + "/" + std::to_string(totalSpace);
            //UIArmyAmount[id]->Text = std::to_string(armyAmount[id]);
            //AddNewArmy(id ,ArmyImage[id], spaceCost);
        ...
        if (usedSpace - spaceCost >= 0  && armyAmount[id] > 0)
        {
            usedSpace -= spaceCost;
            armyAmount[id]--;
            //UISpaceUsage->Text = "Space: " + std::to_string(usedSpace) + "/" + std::to_string(totalSpace);
